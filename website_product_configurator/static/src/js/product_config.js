odoo.define("website_product_configurator.website_form", function (require) {
  ...
  $("button.cfg_clear").on("click", function (event) {
    var path = window.location.pathname;
    ajax.jsonRpc(path + "/config_clear", "call", {}).then(function (res) {
      if (res) {
  ...
  function update_config_image(cfg_vals) {
    ajax
      .jsonRpc(path + "/image_update", "call", {cfg_vals: cfg_vals})
      .then(function (res) {
        if (res) {
  ...
  function update_price(onchange_vals) {
    ...
    var price_labels = $.map(onchange_vals.prices.vals, function (value, i) {
      ...
      dom_prices.append(
        $("<div>").addClass("col-sm-4 text-right").append(price, currency)
      );
  ...
  function value_onchange(cfg_vals) {
    ...
    ajax
      .jsonRpc(path + "/value_onchange", "call", {cfg_vals: cfg_vals})
      .then(function (res) {
        if (res) {
          var select_fields = config_form.find("select.cfg_input");
          if (select_fields) {
            var options = select_fields
              .children("option")
              .not(':empty,[value="custom"]');
            options.addClass("hidden");
            var available_options = options.filter(
              '[value="' + res.value_ids.join('"],[value="') + '"]'
            );
            ...
            select_fields.each(function () {
              ...
              $(this).prop("disabled", !select_options.length);
            });
          }
  ...
  function onchange() {
    ...
    if (cfg_input.is(":checked")) {
      if (cfg_input.attr("type") == "radio") {
        var radio_inputs = config_form.find(
          "input[name='" + cfg_input.attr("name") + "']"
        );
  ...
  $(".cfg_tooltip")
    ...
    .mousemove(function (e) {
      ...
      $(".attr_description").css({top: mousey, left: mousex});
    });
  ...
  config_form.on("click", "a.js_add_qty", function (ev) {
    ...
    var quantity =
      ($link.has(".fa-minus").length ? -1 : 1) + parseFloat($input.val(), 10);
    $input.val(quantity > min ? (quantity < max ? quantity : max) : min);
    $('input[name="' + $input.attr("name") + '"]').val(
      quantity > min ? (quantity < max ? quantity : max) : min
    );
    $input.change();
    return false;
  });
