...
class ReportStockForecat(models.Model):
    ...
    def init(self):
        ...
        SELECT
            ...
            sum(sum(product_qty)) OVER (PARTITION BY product_id ORDER BY date)
        ...
        FROM (
            SELECT
                ...
                SUB.date as date,
                CASE WHEN MAIN.date = SUB.date THEN sum(MAIN.product_qty)
            ...
            FROM (
                SELECT
                    MIN(sq.id) as id,
                    sq.product_id,
                    date_trunc('week', to_date(to_char(CURRENT_DATE,
                    'YYYY/MM/DD'), 'YYYY/MM/DD')) as date,
                ...
                SELECT
                    ...
                    CASE WHEN sm.date_expected > CURRENT_DATE
                        ...
                        to_date(to_char(sm.date_expected,
                        'YYYY/MM/DD'),
                        'YYYY/MM/DD'))
                        ELSE date_trunc('week',
                ...
                FROM
                    stock_move as sm
                    LEFT JOIN product_product
                    ON product_product.id = sm.product_id
                    LEFT JOIN product_template
                    ON product_template.id = product_product.product_tmpl_id
                ...
                GROUP BY
                    sm.date_expected,
                    sm.product_id, product_template.categ_id,
                    product_template.sale_ok
            ) as MAIN
        LEFT JOIN (
