...
class ShipmentScheduleReportCompute(models.TransientModel):
    ...
    def _get_product_ids(self, category_id):
            ...
            for categ in categs:
                if categ.child_id:
                    for child_categ in categ.child_id:
                        categs.append(
                            child_categ
                        ) if child_categ not in categs else categs
        ...
        if not prod_ids:
            raise Warning(
                _(
                    "There is no product to meet the condition (i.e. "
    ...
    def _get_lines(self, periods, category_id):
        ...
        for prod in products:
            ...
            line_vals[prod.id] = {
                ...
                "in2": 0.0,
                "out2": 0.0,
                "bal2": 0.0,  # first period in output
                "in3": 0.0,
                "out3": 0.0,
                "bal3": 0.0,  # second period in output
                "in4": 0.0,
                "out4": 0.0,
                "bal4": 0.0,  # third period in output
                "in5": 0.0,
                "out5": 0.0,
                "bal5": 0.0,  # fourth period in output
                "in6": 0.0,
                "out6": 0.0,
                "bal6": 0.0,  # fifth period in output
            }
