...
class SaleOrder(models.Model):
    ...
    def _generate_and_validate_invoice(self):
        ...
        created_invoice = self.with_context(**ctx_company).action_invoice_create()
        created_invoice = (
            self.env["account.invoice"]
            .browse(created_invoice)
            .with_context(**ctx_company)
        )

        if created_invoice:
            _logger.info(
                "Auto-generated invoice %s (ID %s) for %s (ID %s)",
            ...
            _logger.warning(
                "Could not auto-generate invoice for %s (ID %s)", self.name, self.id
            )
    ...
    def write(self, vals):
        if "team_id" in vals:
            ...
            if vals["team_id"]:
                invoice_policy = (
                    self.env["crm.team"].browse(vals["team_id"]).invoice_policy
                )
