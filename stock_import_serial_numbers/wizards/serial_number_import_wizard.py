...
class SerialNumberMethodWizard(models.TransientModel):
    ...
    def import_method(self):
        ...
        """
        self.ensure_one()
        import_wizard = "stock_import_serial_numbers.serial_number_import_wizard_form"
        return {
...
class SerialNumberImportWizard(models.TransientModel):
    ...
    import_limit = fields.Integer(
        "Import Limit", default=lambda self: self.get_import_limit()
    )
    ...
    def import_serial_numbers(self):
        ...
        if amount_of_lines > self.import_limit:
            raise exceptions.Warning("Maximum %d rows allowed" % self.import_limit)
        ...
        for line in lines:
            if operation.pack_lot_ids.filtered(lambda lot: lot.lot_name == line):
                existing_lines.append(line)
                continue
            production_lot = ProductionLot.create(
                {"name": line, "product_id": operation.product_id.id,}
            )
            OperationLot.create(
                {
                    "qty": 1,
        ...
        if existing_lines:
            raise exceptions.Warning(
                "The following serial numbers are already"
                " in the system:\n%s" % "\n".join(existing_lines)
            )
