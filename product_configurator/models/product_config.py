...
class ProductConfigLine(models.Model):
    ...
    attr_line_val_ids = fields.Many2many(
        comodel_name="product.attribute.value", related="attribute_line_id.value_ids"
    )
...
class ProductConfigStepLine(models.Model):
    ...
    def _check_config_step(self):
        ...
        cfg_steps = cfg_step_lines.filtered(lambda l: l != self).mapped(
            "config_step_id"
        )
...
class ProductConfigSession(models.Model):
    ...
    price = fields.Float(compute="_compute_cfg_price", string="Price", store=True,)
...
class ProductConfigSessionCustomValue(models.Model):
    ...
    def unique_attribute(self):
        if (
            len(
                self.cfg_session_id.custom_value_ids.filtered(
    ...
    def check_custom_type(self):
        ...
        if self.value and custom_type == "binary":
            raise ValidationError(
                _(
                    "Attribute custom type is binary, attachments are the only "
