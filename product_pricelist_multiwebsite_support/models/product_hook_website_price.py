...
def _website_price(self):
    ...
    if (
        ...
        and request.uid != partner.id
    ):
        pricelist = self.env["res.users"].browse(request.uid).property_product_pricelist
    # Added by QTL <<<
    ...
    ret = (
        self.env.user.has_group("sale.group_show_price_subtotal")
        and "total_excluded"
        or "total_included"
    )
