...
class TestPurchaseStockPickingReturnInvoicing(SavepointCase):
    ...
    def test_purchase_stock_return_1(self):
        ...
        self.assertEqual(
            inv_qty, 0.0, ("Quantity invoiced is %s. Should " "be 0.0.") % inv_qty
        )

        # Check quantity received
        rec_qty = sum(pol.qty_received for pol in po.order_line)
        self.assertEqual(
            rec_qty, 0.0, ("Quantity received is %s. Should be " "0.0.") % rec_qty
        )
        # Check quantity returned
        rec_qty = sum(pol.qty_returned for pol in po.order_line)
        self.assertEqual(
            rec_qty, 0.0, ("Quantity returned is %s. Should be " "0.0.") % rec_qty
        )
        ...
        self.assertEqual(
            ...
            5.0,
            (
                "Purchase Stock: received quantity "
        ...
        self.assertEqual(
            ...
            0.0,
            (
                "Purchase Stock: returned quantity "
        ...
        self.assertEqual(
            inv_qty, 5.0, ("Quantity to invoice is %s. Should " "be 5.0.") % inv_qty
        )
        ...
        self.assertEqual(
            inv_qty, 0.0, ("Quantity to refund is %s. Should " "be 0.0.") % inv_qty
        )

        # Check quantity refunded
        inv_qty = sum(pol.qty_refunded for pol in po.order_line)
        self.assertEqual(
            inv_qty, 0.0, ("Quantity refunded is %s. Should " "be 0.0.") % inv_qty
        )
        ...
        inv_qty = sum(pol.qty_to_invoice for pol in po.order_line)
        self.assertEqual(
            inv_qty, 0.0, ("Quantity to invoice is %s. Should " "be 0.0.") % inv_qty
        )

        # Check quantity invoiced
        inv_qty = sum(pol.qty_invoiced for pol in po.order_line)
        self.assertEqual(
            inv_qty, 5.0, ("Quantity invoiced is %s. Should " "be 5.0.") % inv_qty
        )
        ...
        self.assertEqual(
            ...
            2.0,
            (
                "Purchase Stock: quantity returned "
        ...
        inv_qty = sum(pol.qty_to_refund for pol in po.order_line)
        self.assertEqual(
            inv_qty, 2.0, ("Quantity to refund is %s. Should " "be 2.0.") % inv_qty
        )
        ...
        inv_qty = sum(pol.qty_refunded for pol in po.order_line)
        self.assertEqual(
            inv_qty, 2.0, ("Quantity refunded is %s. Should " "be 2.0.") % inv_qty
        )
        ...
        self.assertEqual(
            inv_2.amount_untaxed_signed, inv_1.amount_untaxed_signed / 5 * -2
        )
    ...
    def test_purchase_stock_return_2(self):
        ...
        rec_qty = sum(pol.qty_returned for pol in po.order_line)
        self.assertEqual(
            rec_qty, 0.0, ("Quantity returned " "should be 0.0 instead of %s") % rec_qty
        )

        # Check quantity received
        rec_qty = sum(pol.qty_received for pol in po.order_line)
        self.assertEqual(
            rec_qty, 0.0, ("Received quantity " "should be 0.0 instead of %s") % rec_qty
        )
        ...
        rec_qty = sum(pol.qty_received for pol in po.order_line)
        self.assertEqual(
            rec_qty, 5.0, ("Received quantity " "should be 5.0 instead of %s") % rec_qty
        )
        ...
        rec_qty = sum(pol.qty_returned for pol in po.order_line)
        self.assertEqual(
            rec_qty, 2.0, ("Quantity returned " "should be 2.0 instead of %s") % rec_qty
        )

        # Check quantity to invoice
        inv_qty = sum(pol.qty_to_invoice for pol in po.order_line)
        self.assertEqual(
            inv_qty, 3.0, ("Quantity to invoice is %s. Should " "be 3.0.") % inv_qty
        )
        ...
        inv_qty = sum(pol.qty_invoiced for pol in po.order_line)
        self.assertEqual(
            inv_qty, 3.0, ("Quantity invoiced is %s. Should " "be 3.0.") % inv_qty
        )
        ...
        self.assertEqual(
            ...
            inv_1.amount_untaxed_signed,
            "Purchase Stock: amount invoiced should be 3/5 of the amount " "ordered",
        )
        inv_1.signal_workflow("invoice_open")
