...
class PurchaseOrder(models.Model):
    ...
    def action_view_invoice_refund(self):
        ...
        result["context"] = {"type": "in_refund", "default_purchase_id": self.id}
...
class PurchaseOrderLine(models.Model):
    ...
    def _compute_qty_to_invoice(self):
        ...
        for line in self:
            ...
            if (
                ...
                and not line.move_ids.filtered(lambda x: x.state == "done")
            ):
            ...
            if (
                abs(float_compare(line.qty_to_invoice, 0.0, precision_digits=precision))
                == 1
            ):
                line.invoice_status = "to invoice"
            elif (
                abs(float_compare(line.qty_to_refund, 0.0, precision_digits=precision))
                == 1
            ):
                line.invoice_status = "to invoice"
            elif (
                float_compare(line.qty_to_invoice, 0.0, precision_digits=precision) == 0
                and float_compare(line.qty_to_refund, 0.0, precision_digits=precision)
                == 0
            ):
    ...
    qty_refunded = fields.Float(
        compute="_compute_qty_refunded", string="Refunded Qty", copy=False, default=0.0
    )

    qty_returned = fields.Float(
        compute="_compute_qty_returned", string="Returned Qty", copy=False, default=0.0
    )
