...
def _cart_update(self, product_id=None, line_id=None, add_qty=0, set_qty=0,
    ...
    if self.state != 'draft':
        ...
        raise UserError(_(
            'It is forbidden to modify a sale order which is not in draft status'))
    ...
    if not order_line:
        ...
        values['name'] = self._get_line_description(self.id, product_id,
    ...
    if quantity <= 0:
        ...
        values = {
            ...
            'order_id': self.id,
            'product_uom':
                self.env['product.product'].browse(product_id).uom_id.id,
        }  # QTL ADD
