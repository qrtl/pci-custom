...
class SaleCategoryDiscount(common.TransactionCase):
    ...
    def setUp(self):
        ...
        self.delivery_method = self.env["delivery.carrier"].create(
            dict(name="Test Delivery", delivery_type="base_on_rule",)
        )
        self.delivery_rule_1 = self.env["delivery.price.rule"].create(
            dict(
        ...
        self.delivery_rule_2 = self.env["delivery.price.rule"].create(
            dict(
                carrier_id=self.delivery_method.id,
                variable="price",
                operator="<",
