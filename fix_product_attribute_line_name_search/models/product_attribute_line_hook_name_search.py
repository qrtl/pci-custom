...
def name_search(self, name='', args=None, operator='ilike', limit=100):
    ...
    if name and operator in ('=', 'ilike', '=ilike', 'like', '=like'):
        ...
        # ('value_ids', operator, name)]  # QTL del
        args = args or []  # QTL add
        domain = ['|', ('attribute_id', operator, name),
                  ('value_ids', operator, name)]  # QTL add
        return self.search(expression.AND([domain, args]),
                           limit=limit).name_get()  # QTL add
    return super(ProductAttributeLine, self).name_search(name=name, args=args,
                                                         operator=operator,
                                                         limit=limit)
