    ...
    <body>
        ...
        %for inv in objects :
            ...
            %endif
                ...
                <tr>
                    <td style="text-align:right;">${_("Datum")} </td><td  style="text-align:right;">
                        ${formatLang(inv.date_invoice, date=True)|entity}
                    </td>
                </tr>
            </table>
            <br/>
                ...
                <tbody>
                    %for line in inv.invoice_line_sorted :
                        <tr>
                            ...
                            %endif
                            <td>
                                ${line.name or line.product_id.name |entity}
                                %if line.note and len(line.note.replace('\n','')) > 0 :
                                    <p class="note">${line.note |carriage_returns}</p>
                                %endif
                            </td>
                            %if inv.print_ean:
                            ...
                            %endif
                            <td style="white-space:nowrap;text-align:right;">${formatLang(line.price_subtotal)}</td>
                        </tr>
                    %endfor
                        <tr>
                        ...
                        <tr>
                            <td colspan="${inv.cols}" style="border-style:none"/>
                            <td ><b>${_("Taxes")}:</b></td>
                            <td style="text-align:right">${formatLang(inv.amount_tax)}</td>
                        </tr>
                        <tr> 
                            <td colspan="${inv.cols}" style="border-style:none"/>
                            <td style="border:2px solid;font-weight:bold;white-space:nowrap">${_("Total")} ${inv.currency_id.name}:</td>
                            <td style="border:2px solid;text-align:right;font-weight:bold">${formatLang(inv.amount_total)}</td>
                        </tr>
                </tbody>
            </table>
            <br>
