...
class account_invoice(osv.osv):
    ...
    def _print_serial_id(self, cr, uid, ids, name, args, context=None):
        res = {}
        invoice_line_obj = self.pool.get('account.invoice.line')
        for invoice in self.browse(cr, uid, ids, context=context):
          print_serial_id = False
          if not 'serial_id' in invoice_line_obj._columns:
            res[invoice.id] = False
          elif invoice.invoice_line:
            for line in invoice.invoice_line:
                if line.serial_id:
                    print_serial_id = True
          res[invoice.id] = print_serial_id
        return res
    ...
    _columns = {
        ...
        'print_serial_id': fields.function(_print_serial_id, method=True, type='boolean', string='Print serial if available',),
