...
class AuditlogLog(models.Model):
    ...
    model_id = fields.Many2one("ir.model", string=u"Model")
    res_id = fields.Integer(u"Resource ID")
    user_id = fields.Many2one("res.users", string=u"User")
    method = fields.Char(u"Method", size=64)
    line_ids = fields.One2many("auditlog.log.line", "log_id", string=u"Fields updated")
    http_session_id = fields.Many2one("auditlog.http.session", string=u"Session")
    http_request_id = fields.Many2one("auditlog.http.request", string=u"HTTP Request")
    log_type = fields.Selection(
        [("full", u"Full log"), ("fast", u"Fast log"),], string=u"Type"
    )
