    ...
    def _render_patched(self, result_text="test result", call_count=1):
        ...
        with mock.patch.object(
            ...
            patched_pdf.side_effect = (
                lambda record, data, save_attachments: py3o_report._postprocess_report(
                    result, record.id, save_attachments,
                )
                or result
            )
            # test the call the the create method inside our custom parser
            self.report.render_report(self.env.user.ids, self.report.report_name, {})
            self.assertEqual(call_count, patched_pdf.call_count)
            # generated files no more exists
            self.assertFalse(os.path.exists(result))

    def test_reports(self):
        res = self.report.render_report(self.env.user.ids, self.report.report_name, {})
        self.assertTrue(res)
