...
class Py3oReport(models.TransientModel):
    ...
    def get_template(self, model_instance):
        ...
        if tmpl_data is None:
            ...
            raise TemplateNotFound(
                _(u"No template found. Aborting."), sys.exc_info(),
            )
    ...
    def _get_parser_context(self, model_instance, data):
        report_xml = self.ir_actions_report_xml_id
        context_instance = rml_parse(
            self.env.cr, self.env.uid, report_xml.name, context=self.env.context
        )
        context_instance.set_context(
            model_instance, data, model_instance.ids, report_xml.report_type
        )
    ...
    def _create_single_report(self, model_instance, data, save_in_attachment):
        ...
        if len(model_instance) == 1:
            self._postprocess_report(result_path, model_instance.id, save_in_attachment)
    ...
    def _convert_single_report(self, result_path, model_instance, data):
        ...
        if not Formats().get_format(filetype).native:
            ...
            result_path = os.path.join(
                result_path, "{}.{}".format(os.path.splitext(result_filename)[0], filetype)
            )
        return result_path
    ...
    def _get_or_create_single_report(self, model_instance, data, save_in_attachment):
        self.ensure_one()
        if save_in_attachment and save_in_attachment["loaded_documents"].get(
            model_instance.id
        ):
    ...
    def create_report(self, res_ids, data):
        ...
        save_in_attachment = (
            self._check_attachment_use(res_ids, self.ir_actions_report_xml_id) or {}
        )
        reports_path = []
        if len(res_ids) > 1 and self.ir_actions_report_xml_id.py3o_multi_in_one:
            reports_path.append(
                self._create_single_report(model_instances, data, save_in_attachment)
            )
        else:
