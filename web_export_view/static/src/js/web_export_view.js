  ...
  Sidebar.include({
    redraw: function () {
      ...
      if (self.getParent().ViewManager.active_view.type == "list") {
        self.$el
          .find(".o_dropdown")
          .last()
          .append(QWeb.render("WebExportTreeViewXls", {widget: self}));
        self.$el
    ...
    on_sidebar_export_treeview_xls: function () {
      ...
      $.each(view.visible_columns, function () {
        if (
          this.tag == "field" &&
          (this.widget === undefined || this.widget != "handle")
        ) {
      ...
      if (children) {
        ...
        view.$el
          .find(
            ".o_list_view > tbody > tr[data-id]:has(.o_list_record_selector input:checkbox:checked)"
          )
          .each(function () {
            var $row = $(this);
            var export_row = [];
            $.each(export_columns_keys, function () {
              ...
              if ($cellcheckbox.length) {
                export_row.push(
                  $cellcheckbox.is(":checked") ? _t("True") : _t("False")
                );
              } else {
