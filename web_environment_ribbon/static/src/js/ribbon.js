...
odoo.define("web_environment_ribbon.ribbon", function (require) {
  ...
  core.bus.on("web_client_ready", null, function () {
    ...
    $("body").append(ribbon);
    ribbon.hide();
    model.call("get_param", ["ribbon.name"]).then(function (name) {
      if (name && name != "False") {
        ribbon.html(name);
        ribbon.show();
      }
    ...
    model.call("get_param", ["ribbon.color"]).then(function (strColor) {
    ...
    model
      .call("get_param", ["ribbon.background.color"])
      .then(function (strBackgroundColor) {
