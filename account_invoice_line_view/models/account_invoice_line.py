...
class AccountInoviceLine(models.Model):
    ...
    type = fields.Selection(related="invoice_id.type", store=True, readonly=True,)
    user_id = fields.Many2one(
        related="invoice_id.user_id", store=True, string="Salesperson"
    )
    ...
    reference = fields.Char(
        compute="_compute_get_reference", store=True, string="Invoice Ref"
    )
    ...
    rate = fields.Float(compute="_get_base_amt", store=True, string="Rate")
    base_amt = fields.Monetary(
        compute="_get_base_amt", store=True, currency_field="company_currency_id",
    )
    partner_id = fields.Many2one(
        related="invoice_id.partner_id", string="Customer", store=True
    )
        ...
        for l in self:
            ...
            if l.company_currency_id == l.currency_id:
                ...
                l.rate = l.currency_id.with_context(
                    dict(l._context or {}, date=l.date_invoice)
                ).rate
