...
class ReportStockForecat(models.Model):
    ...
    # Fully Override from base to add Product Category and Can Be Sold to report
    @api.model_cr
    def init(self):
        ...
        (SELECT
            ...
            GROUP BY date, sq.product_id, product_template.categ_id, product_template.sale_ok
            UNION ALL
            SELECT
            MIN(-sm.id) as id,
            sm.product_id,
            ...
            GROUP BY sm.date_expected,sm.product_id, product_template.categ_id, product_template.sale_ok
            UNION ALL
            SELECT
                MIN(-sm.id) as id,
                sm.product_id,
    ...
    GROUP BY product_id,date,categ_id,sale_ok)""")
