...
class AcquirerPaypal(models.Model):
    ...
    def paypal_form_generate_values(self, values):
        paypal_tx_values = super(AcquirerPaypal, self).paypal_form_generate_values(
            values
        )
        base_url = ""
        ...
        paypal_tx_values.update(
            {
                "paypal_return": "%s"
                % urlparse.urljoin(base_url, PaypalController._return_url),
                "notify_url": "%s"
                % urlparse.urljoin(base_url, PaypalController._notify_url),
                "cancel_return": "%s"
                % urlparse.urljoin(base_url, PaypalController._cancel_url),
            }
        )
