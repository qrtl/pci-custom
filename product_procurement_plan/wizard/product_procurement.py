...
class ProductProcInfoCompute(models.TransientModel):
                ...
                if bom:
                    for l in bom.bom_line_ids:
                        if adjust and l.product_id.avg_qty_adj:
                            continue
                        if (
                            not l.attribute_value_ids
                            or l.attribute_value_ids in prod.attribute_value_ids
                        ):
    ...
    def _update_avg_qty_needed(
        ...
        for prod in self.env["product.product"].browse(list(qty_dict.keys())):
            if (
                prod.id in curr_qty_dict
                and qty_dict[prod.id] / months == curr_qty_dict[prod.id]
            ):
                continue
            else:
    ...
    def _get_sorted_products(self, sorted_products):
        ...
        manufacture_route_id = self.env[
            "stock.warehouse"
        ]._get_manufacture_route_id()  # integer
        ...
        for prod in sorted_products:
            ...
            elif manufacture_route_id in [r.id for r in prod.product_tmpl_id.route_ids]:
    ...
    def _update_buy_prod_procure_lt(self, buy_prod_dict, curr_lt_dict, from_date):
        # work on buy_prod_dict and update procurement lead time in db
        loc_obj = self.env["stock.location"]
        int_loc_ids = [l.id for l in loc_obj.search([("usage", "=", "internal")])]
        supp_loc_ids = [l.id for l in loc_obj.search([("usage", "=", "supplier")])]
    ...
    def _get_bom_products(self, products):
        ...
        for rec in bom_recs:
            if (
                rec.product_id
                and rec.product_id in products
                and rec.product_id not in bom_products
            ):
    ...
    def _get_sorted_parent_products(self):
        ...
        for prod in bom_products:
            bom_lines = self.env["mrp.bom.line"].search([("product_id", "=", prod.id)])
            if bom_lines:
                ok_flag = False
                for l in bom_lines:
    ...
    def product_procure_calc(self):
        ...
        months = self.env["res.company"].browse(company_id).procurement_calc_months
        from_date = (datetime.today() + relativedelta(days=-months * 30)).strftime(
        ...
        if sorted_parent_products and self.average_qty_adj:
            self._update_avg_qty_needed(sorted_parent_products, from_date, months, True)
