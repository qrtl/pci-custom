    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        for move in self.browse(cr, uid, ids, context):
            if move.product_id.product_tmpl_id.categ_id.enforce_qty_1 \
                ...
                raise \
                    osv.except_osv(_('Error!'), _(
                        'Quantity of stock move should be'
                        ' 1 for product %s (enforce quantity 1).'
                    ) % move.product_id.name)
            if move.sale_line_id \
                    and not move.sale_line_id.serial_id and move.prodlot_id:
                self.pool.get('sale.order.line').write(
                    ...
                    context)
                invoice_line_ids = [line.id for line in
                                    move.sale_line_id.invoice_lines]
