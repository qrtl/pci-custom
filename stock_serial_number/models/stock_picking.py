        ...
        for picking in pickings:
            ...
            for serial in serial_dict:
                if serial_dict[serial] > 1:
                    serial_data = \
                        self.pool.get('stock.production.lot').browse(cr, uid,
                                                                     [serial],
                                                                     context)[
                            0]
                    if serial_data.product_id.product_tmpl_id.categ_id.enforce_qty_1:
                        raise osv.except_osv(_('Error!'),
                                             _(
                                                 'Sorry, You can not have serial number same for multiple move lines - [%s]')
                                             % serial_data.name)
        return super(stock_picking, self).action_done(cr, uid, ids,
                                                      context=context)
