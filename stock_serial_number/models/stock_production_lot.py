...
class StockProductionLot(models.Model):
    ...
    def _default_lb_unit(self):
        ...
        try:
            res = md.get_object_reference("product", "product_uom_lb")[1]
        except ValueError:
            res = False
        return res
    ...
    def _default_kg_unit(self):
        md = self.env["ir.model.data"]
        try:
            res = md.get_object_reference("product", "product_uom_kgm")[1]
        except ValueError:
            res = False
        return res
    ...
    product_name = fields.Char(
        related="product_id.name", string="Product", readonly=True
    )
    prefix = fields.Char()
    ...
    model_id = fields.Many2one(comodel_name="stock.model", string="Model",)
    sequence = fields.Integer(
        related="model_id.sequence",
        # store=True,
    )
    body_id = fields.Many2one(comodel_name="stock.body", string="Body")
    neck_id = fields.Many2one(comodel_name="stock.neck", string="Neck")
    pickguard_id = fields.Many2one(comodel_name="stock.pickguard", string="Pickguard")
    shop_ids = fields.Many2many(comodel_name="stock.shop", string="Shop",)
    check = fields.Selection([("deliver", u"★"), ("rework", u"▲")],)
    ...
    stock_available = fields.Float(
        compute="_compute_balance", store=True, string="Available", readonly=True,
    )
    ...
    def _get_reserved_qty(self):
        for lot in self:
            quants = self.env["stock.quant"].search(
                [("lot_id", "=", lot.id), ("reservation_id", "!=", False)]
            )
    ...
    def name_search(self, name, args=None, operator="ilike", limit=100):
        ...
        domain = []
        if name:
            domain = ["|", ("name", operator, name), ("prefix", operator, name)]
