...
def _get_signup_url_for_action(
    self, action=None, view_type=None, menu_id=None, res_id=None, model=None
):
    ...
    for partner in self:
        ...
        signup_type = self.env.context.get(
            "signup_force_type_in_url", partner.signup_type or ""
        )
        if signup_type:
        ...
        res[partner.id] = urljoin(
            base_url, "/web/{}?{}".format(route, werkzeug.url_encode(query))
        )
    return res
